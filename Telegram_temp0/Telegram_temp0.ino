...
void loop() {
  ...
  // 불쾌지수 계산
  float di = (9.0 / 5.0 * temp) - (0.55 * (1.0 - humid / 100.0) * (9.0 / 5.0 * temp - 26.0)) + 32.0;
  Serial.print("Discomfort Index (DI): "); Serial.println(di); // 불쾌지수 출력
  ...
  // 메시지 생성
  String msg = "온도는 " + String(temp) + "\xC2\xB0C 그리고 습도는 " + String(humid) + "%\n불쾌지수는 " + String(di) + "입니다.";
